...
$$
\dot{\mathbf{x}} = f_{IMU}(\mathbf{x}, \mathbf{u})
$$
...
- Calcolo dello stato predetto:
    $$
    \dot{\hat{\mathbf{x}}} =  \hat{f}_{IMU}
    $$
    $$
    \hat{\mathbf{x}}_{t+1|t} = \hat{\mathbf{x}}_{t|t} + f_{IMU}(\hat{\mathbf{x}}_{t|t}, \mathbf{u}) \cdot dt
    $$

- Propagazione covarianza: 

    $$
    \dot{\mathbf{P}} = \mathbf{F} \mathbf{P} \mathbf{F}^T +  \mathbf{G} \mathbf{Q}_{IMU} \mathbf{G}^T
    $$
    $$
    \mathbf{P}_{t+1|t} = \mathbf{P}_{t|t} + \dot{\mathbf{P}} \cdot dt
    $$
...
- Calcola la predizione della misura:
    $$
    \hat{\mathbf{z}} = h(\hat{\mathbf{x}}_{t+1|t})
    $$

- Covarianza dell'innovazione:
    $$
    \mathbf{S} = \mathbf{H} \mathbf{P}_{t+1|t} \mathbf{H}^T + \mathbf{R}
    $$

- Calcolo del guadagno di Kalman:
    $$
    \mathbf{K} = \mathbf{P}_{t+1|t} \mathbf{H}^T \mathbf{S}^{-1}
    $$

## Aggiornamento dello stato
- Aggiornamento di stato:
    $$
    \hat{\mathbf{x}}_{t+1|t+1} = \hat{\mathbf{x}}_{t+1|t} + \mathbf{K} (\mathbf{z}_{meas} - \hat{\mathbf{z}})
    $$

- Aggiornamento della covarianza
    $$
    \mathbf{P}_{t+1|t +1} = ( \mathbf{I} - \mathbf{K} \mathbf{H} ) \mathbf{P}_{t+1|t} 
    $$
...
- Questi sono ri-calcolati ad ogni passo del filtro EKF, utilizzando la stima corrente dello stato.
